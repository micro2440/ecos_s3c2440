...
# <cyg/io/framebufs/mini2440_fb.h> and mini2440_fbs.c
#
#####DESCRIPTIONEND####
#===============================================================================

proc do_it { prefix } {
    set pkgconf_file	[file join $prefix "include/pkgconf/devs_framebuf_mini2440.h"]
    if { ! [file exists $pkgconf_file] } {
        puts stderr "gen_mini2440fb.tcl: missing configuration header $pkgconf_file"
        exit 1
    }
    ...
    set data \
        "/*
 * File <cyg/io/framebufs/mini2440_fb.h>
 *
...
"
	...
	set fb 0
        if { ! [info exists defines(CYGPKG_DEVS_FRAMEBUF_MINI2440_FB[set fb])] } {
            continue
        }
        
        set width	$defines(CYGNUM_DEVS_FRAMEBUF_MINI2440_FB[set fb]_WIDTH)
        set height	$defines(CYGNUM_DEVS_FRAMEBUF_MINI2440_FB[set fb]_HEIGHT)
        set flags0  "(CYG_FB_FLAGS0_LINEAR_FRAMEBUFFER | \\\n    CYG_FB_FLAGS0_DOUBLE_BUFFER | \\\n"
        if { [info exists defines(CYGIMP_DEVS_FRAMEBUF_MINI2440_FB[set fb]_VIEWPORT) ] } {
            set viewport_width	$defines(CYGNUM_DEVS_FRAMEBUF_MINI2440_FB[set fb]_VIEWPORT_WIDTH)
            set viewport_height	$defines(CYGNUM_DEVS_FRAMEBUF_MINI2440_FB[set fb]_VIEWPORT_HEIGHT)
            append flags0 "    CYG_FB_FLAGS0_VIEWPORT | \\\n"
        } else {
            set viewport_width  $width
            set viewport_height $height
        }
        if { [info exists defines(CYGNUM_DEVS_FRAMEBUF_MINI2440_FB[set fb]_PAGE_FLIPPING)] } {
            append flags0 "    CYG_FB_FLAGS0_PAGE_FLIPPING | \\\n"
        }
        set paletted		1
        switch -- $defines(CYGDAT_DEVS_FRAMEBUF_MINI2440_FB[set fb]_FORMAT) {
            1BPP_BE_PAL888 {
        ...
        if { $paletted } {
            ...
            set write_palette_fn	"cyg_mini2440_fb_write_palette"
            set read_palette_fn		"cyg_mini2440_fb_read_palette"
...
"
    ...
    set old_data ""
    set framebuf_file [file join $prefix "include/cyg/io/framebufs/mini2440_fb.h"]
...
if { 0 == $::argc } {
    puts stderr "gen_mini2440fb.tcl: missing argument for install directory"
    exit 1
}
if { [catch { do_it [lindex $::argv 0] } msg] } {
    puts stderr "gen_mini2440fb.tcl: internal error"
    puts stderr "    $msg"
    exit 1
}
exit 0
