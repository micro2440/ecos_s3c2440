...
handle_X_event(void)
{
    ...
    switch(event.type) {
      ...
      default:
        DEBUG(0, "X event: unsupported type %d\n", event.type);
        break;
    }
}
...
handle_mapped(int winid)
{
    ...
    DEBUG(0, "host_win 0x%x attr.root 0x%x\n", (int)host_win, (int)attr.root);
    DEBUG(0, "XSelectInput returns %d\n", result);
    if (0 && result != Success) {
        error("XSelect error %d.", result);
    }
