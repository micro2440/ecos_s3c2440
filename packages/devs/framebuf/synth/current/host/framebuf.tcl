...
namespace eval framebuf {
    ...
    variable    SYNTH_FB_TS_SETUP   0x05
    variable    SYNTH_FB_TS_EVENT   0x06
    ...
    # Protocol for touchscreen event types
    variable	SYNTH_FB_TS_UP      0x00
    variable	SYNTH_FB_TS_MOTION  0x03
    variable	SYNTH_FB_TS_DOWN    0x04
	...
	} elseif { $framebuf::SYNTH_FB_TS_SETUP == $reqcode } {
	    DEBUG 1 $devid "send touchscreen interrupt setup response\n"
	    set len [string length $framebuf::settings($devid,ts_interrupt)]
            DEBUG 2 $devid "framebuf ($devid) send ts setup response; length $len value $framebuf::settings($devid,ts_interrupt)"
	    synth::send_reply $reqcode $len $framebuf::settings($devid,ts_interrupt)
	} elseif { $framebuf::SYNTH_FB_TS_EVENT == $reqcode } {
	    set len [string length $framebuf::settings($devid,mouse)]
            DEBUG 2 $devid "send mouse event reply length $len: $framebuf::settings($devid,mouse)"
	    synth::send_reply $reqcode $len $framebuf::settings($devid,mouse)
        } else {
            synth::report_error "framebuf: got invalid request $reqcode from eCos application"
        }
    }

    proc mouse_press { devid x y } {
        # DEBUG 0 $devid "mouse clicked ($x,$y)"
        # synth::report_error "framebuf: mouse clicked ($x, $y)"
        # puts stderr "framebuf ($devid): mouse clicked ($x, $y)"
	if { [info exists framebuf::settings($devid,ts_interrupt)] } {
	    set framebuf::settings($devid,mouse) [list "$framebuf::SYNTH_FB_TS_DOWN" "$x" "$y"]
	    synth::interrupt_raise $framebuf::settings($devid,ts_interrupt)
	}
    }

    proc mouse_move { devid x y } {
        # DEBUG 0 $devid "mouse clicked ($x,$y)"
        # synth::report_error "framebuf: mouse clicked ($x, $y)"
        # puts stderr "framebuf ($devid): mouse clicked ($x, $y)"
	if { [info exists framebuf::settings($devid,ts_interrupt)] } {
	    set framebuf::settings($devid,mouse) [list "$framebuf::SYNTH_FB_TS_MOTION" "$x" "$y"]
	    synth::interrupt_raise $framebuf::settings($devid,ts_interrupt)
	}
    }

    proc mouse_release { devid x y } {
        # DEBUG 0 $devid "mouse clicked ($x,$y)"
        # synth::report_error "framebuf: mouse clicked ($x, $y)"
        # puts stderr "framebuf ($devid): mouse clicked ($x, $y)"
	if { [info exists framebuf::settings($devid,ts_interrupt)] } {
	    set framebuf::settings($devid,mouse) [list "$framebuf::SYNTH_FB_TS_UP" "$x" "$y"]
	    synth::interrupt_raise $framebuf::settings($devid,ts_interrupt)
	}
    }

    proc mapped { devid } {
        # puts stderr "devid $devid: frame has been mapped [winfo id $framebuf::settings($devid,frame)]"
        DEBUG 1 $devid "frame has been mapped [winfo id $framebuf::settings($devid,frame)]\n"
        bind $framebuf::settings($devid,frame) <Map> ""
        bind $framebuf::settings($devid,frame) <ButtonPress-1> " framebuf::mouse_press $devid %x %y "
        bind $framebuf::settings($devid,frame) <B1-Motion> " framebuf::mouse_move $devid %x %y "
        bind $framebuf::settings($devid,frame) <ButtonRelease-1> " framebuf::mouse_release $devid %x %y "
        ...
        DEBUG 1 $devid "frame has been mapped and redrawn [winfo id $framebuf::settings($devid,frame)]\n"
    }
    
    proc instantiate { devid name data } {

        # puts stderr "instantiate devid $devid, name $name, data $data"
        ...
        # is a protocol number as per ../include/protocol.h
        set junk 	""
        set protocol 	0
	...
	set ts_option "fb$framebuf::settings($devid,fbid)_touchscreen"
	if { [synth::tdf_has_option "framebuf" $ts_option] } {
	    DEBUG 1 $devid "this framebuf has an option $ts_option"
	    set ts_optval [synth::tdf_get_option "framebuf" $ts_option]
	    if { "on" == $ts_optval } {
		# Instantiate a touchscreen interrupt
		set ts_device "ts$framebuf::settings($devid,fbid)"
		set framebuf::settings($devid,ts_interrupt) [synth::interrupt_allocate $ts_device]
		DEBUG 1 $devid "touchscreen interrupt vector $framebuf::settings($devid,ts_interrupt)"
	    }
	}
