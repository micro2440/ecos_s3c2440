...
#ifdef REQUIRE_TOUCHSCREEN
        local_fb_data->ts = cyg_ts_synth_instantiate(shared_fb_data->devid,
                                                     fb->fb_driver0);
        if (local_fb_data->ts == NULL) {
            synth_auxiliary_xchgmsg(shared_fb_data->devid, SYNTH_FB_ABORT, 0, 0, 0, 0, 0, 0, 0, 0);
            cyg_hal_sys_close(fd);
            cyg_hal_sys_close(shared_fb_data->fifo_to_framebuf);
        }
...
#else
#  error I did configure a touchscreen, right
#endif
